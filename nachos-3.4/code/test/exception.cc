    ...
    if (which == NoException)
                ...
                case SC_OpenFileID:
		{
			...
			int i;
                        for (i=0;i<10;i++)
			{
				if (fileSystem->Open(buffer)!=NULL)
				if ((fileSystem->FileList[i]==NULL) && (check==false) &&(fileSystem->countfile<11))
				{
				    fileSystem->FileList[i] = fileSystem->Open(buffer);
				...
				DEBUG('f',"unsucessful");
                        ...
                        {
				DEBUG('f',"Successful");
				machine->WriteRegister(2, fileSystem->FileList[i]->GetFileId());
			};
			delete [] buffer;
			break;
		}
                case SC_CloseFile:
		{
			int bufferAddress = machine->ReadRegister(4); // read string pointer from user
                        ...
                        for (int i=0;i<10;i++)
			{
				if ((check==false) &&(fileSystem->FileList[i]!=NULL)&&(fileSystem->FileList[i]->GetFileId()==bufferAddress))
                                {
                                        check=true;
					delete fileSystem->FileList[i];
                                        fileSystem->type[i]=-1;
				}
			}
                        if (check==true)
